var pageUrl = "/kindergarten/page.html";
var deleteUrl = "/kindergarten/delete.html";
...
function showTable(pageInfo) {
    ...
    for(var i=0,length_1 = beans.length;i<length_1;i++){
        ...
        var tr = "<tr>"
            ...
            +'<td>'+replaceNull(beans[i].server)+'</td>'
            +'<td>'+replaceNull(beans[i].address)+'</td>'
            +'<td>'+replaceNull(beans[i].amount)+'</td>'
            +'<td>';
	        if(isAdmin()){
